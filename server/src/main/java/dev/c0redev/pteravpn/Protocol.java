...
final class Protocol {
  ...
  static final byte MSG_PING = 2;
  static final byte MSG_PONG = 3;

  static final byte ADDR_V4 = 4;
  static final byte ADDR_V6 = 6;

  static final byte FLAG_COMPRESSION = 1;

  static void writeHandshake(OutputStream out, byte role, byte channelId, String token, boolean compression) throws IOException {
    out.write(MAGIC);
    out.write(VERSION);
    out.write(role);
    out.write(compression ? FLAG_COMPRESSION : 0);
  ...
  static Handshake readHandshakeBody(InputStream in) throws IOException {
    ...
    int flags = readU8(in);
    boolean compression = (flags & FLAG_COMPRESSION) != 0;
    ...
    return new Handshake(role, channelId, token, compression);
  }
  ...
  static UdpFrame readUdpFrame(InputStream in) throws IOException {
    ...
    byte msg = bb.get();
    if (msg == MSG_PING || msg == MSG_PONG) return new UdpFrame(msg, (byte) 0, 0, null, 0, new byte[0]);
    ...
    bb.get(payload);
    return new UdpFrame(msg, at, srcPort, dst, dstPort, payload);
  }

  static void writeUdpFrame(OutputStream out, UdpFrame f) throws IOException {
    if (f.msgType() == MSG_PING || f.msgType() == MSG_PONG) {
      writeU32(out, 1);
      out.write(f.msgType() & 0xff);
      out.flush();
      return;
    }
  ...
  record Handshake(byte role, int channelId, String token, boolean compression) {}
  record TcpConnect(byte addrType, InetAddress ip, int port) {}
  record UdpFrame(byte msgType, byte addrType, int srcPort, InetAddress dst, int dstPort, byte[] payload) {}
}
